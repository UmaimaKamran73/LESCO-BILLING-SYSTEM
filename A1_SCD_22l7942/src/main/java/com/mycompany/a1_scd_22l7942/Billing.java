...
class Billing 
{
    String CustID;       //from customers (do aggregation for it?
    Month billingMonth;  //using builtin enum
    int currentRegularMeterReading;
    int currentPeakMeterReading;
    LocalDate readingEntryDate; //can also be called as issue date. CANNOT BE IN FUTURE. formate dd/mm/yyyy 
    BigDecimal electricityCost;
    BigDecimal salesTaxAmount;
    BigDecimal fixedCharges;
    BigDecimal totalBillingAmount;
    LocalDate dueDate;   //7 dates after readingEntryDate. CANNOT BE BEFORE READING DATE format dd/mm/yyyy
    BillStatus billPaidStatus; //enum again. (paid or unpaid)
    LocalDate billPaymentDate;  //dd//mm/yyyycannot be b4 reading entry date
    ...
    public Billing(String CustID, Month billingMonth, int currentRegularMeterReading, int currentPeakMeterReading, LocalDate readingEntryDate, BigDecimal electricityCost, BigDecimal salesTaxAmount, BigDecimal fixedCharges, BigDecimal totalBillingAmount, LocalDate dueDate, BillStatus billPaidStatus, LocalDate billPaymentDate) {
        this.CustID = CustID;
    ...
    public Billing(String CustID, Month billingMonth, int currentRegularMeterReading, int currentPeakMeterReading, LocalDate readingEntryDate, BigDecimal electricityCost, BigDecimal salesTaxAmount, BigDecimal fixedCharges, BigDecimal totalBillingAmount, LocalDate dueDate, BillStatus billPaidStatus) {
        this.CustID = CustID;
        this.billingMonth = billingMonth;
        this.currentRegularMeterReading = currentRegularMeterReading;
        this.currentPeakMeterReading = currentPeakMeterReading;
   ...
   public void viewCurrentBill()
   {
    ...
    public String toString() {
        return "CustID= " + CustID + " Billing Month=" + billingMonth + " Current Regular Meter Reading= " + currentRegularMeterReading + "\nCurrent Peak Meter Reading= " + currentPeakMeterReading + " Reading Entry Date= " + readingEntryDate + "\nElectricity Cost= " + electricityCost + " SalesTaxAmount= " + salesTaxAmount + " Fixed Charges= " + fixedCharges + "\nTotal Billing Amount= " + totalBillingAmount + " Due Date= " + dueDate + " Bill Paid Status= " + billPaidStatus + " Bill Payment Date= " + billPaymentDate;
    }
